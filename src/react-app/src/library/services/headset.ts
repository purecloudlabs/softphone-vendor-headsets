...
export default class HeadsetService {
  ...
  endCall(conversationId: string, hasOtherActiveCalls?: boolean | undefined): Promise<any> {
    const service = this.selectedImplementation;
    if (!service || !service.isConnected) {
  ...
  triggerDeviceAnsweredCall(eventInfo: any) {
  ...
  triggerDeviceRejectedCall(conversationId: string) {
  ...
  triggerDeviceEndedCall(eventInfo: any) {
  ...
  triggerDeviceMuteStatusChanged(isMuted: boolean, eventInfo: any) {
  ...
  triggerDeviceHoldStatusChanged({ holdRequested, toggle }: {holdRequested: boolean, toggle: boolean}, eventInfo: any) {
  ...
  */
  triggerDeviceLogs(eventInfo: any) {
    this.$headsetEvents.next(
